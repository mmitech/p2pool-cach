...
nets = dict(
    ...
    qqcoin=math.Object(
        P2P_PREFIX='d9e6e7f5'.decode('hex'),
        P2P_PORT=20111,
        ADDRESS_VERSION=58, #QQC starts L
        RPC_PORT=20112,
        RPC_CHECK=defer.inlineCallbacks(lambda bitcoind: defer.returnValue(
            'QQcoinaddress' in (yield bitcoind.rpc_help()) and
            not (yield bitcoind.rpc_getinfo())['testnet']
        )),
        SUBSIDY_FUNC=lambda target: 10*100000000,
        BLOCKHASH_FUNC=lambda header: pack.IntType(256).unpack(__import__('yac_scrypt').getPoWHash(header, data.block_header_type.unpack(header)['timestamp'])),
        POW_FUNC=lambda header: pack.IntType(256).unpack(__import__('yac_scrypt').getPoWHash(header, data.block_header_type.unpack(header)['timestamp'])),
        BLOCK_PERIOD=20, # s OMG!
        SYMBOL='QQC',
        CONF_FILE_FUNC=lambda: os.path.join(os.path.join(os.environ['APPDATA'], 'qqcoin') if platform.system() == 'Windows' else os.path.expanduser('~/Library/Application Support/qqcoin/') if platform.system() == 'Darwin' else os.path.expanduser('~/.QQcoin'), 'QQcoin.conf'),
        BLOCK_EXPLORER_URL_PREFIX='http://qqcexplorer.tk/block/', #dummy!
        ADDRESS_EXPLORER_URL_PREFIX='http://qqcexplorer.tk/address/',
        SANE_TARGET_RANGE=(2**256//2**20//1000 - 1, 2**256//2**20 - 1),
    ),
